...
export default function Index({ auth, brands, filters }) {
  const [globalFilter, setGlobalFilter] = useState(filters?.search || '');
  const toast = useRef(null);

  const handleSearch = (e) => {
    e.preventDefault();
    router.get(route('brands.index'), { search: globalFilter }, {
      preserveState: true,
      replace: true
    });
  };
  ...
  const actionTemplate = (rowData) => (
    <div className="flex gap-2">
      <Button
        ...
        onClick={() => router.visit(route('brands.edit', rowData.id))}
      />
      <Button
        icon="pi pi-trash"
  ...
  const tableHeader = (
    <div className="flex flex-wrap align-items-center justify-content-between gap-3">
      ...
      <Button
        ...
        onClick={() => router.visit(route('brands.create'))}
      />
    </div>
  );
        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <Card className="text-center shadow-2">
              <div className="flex align-items-center justify-content-center mb-3">
                <div className="bg-blue-100 p-3 border-round-xl">
                  <i className="pi pi-tags text-blue-500 text-2xl"></i>
                </div>
              </div>
              <span className="text-2xl font-bold text-gray-800">{brands.total}</span>
              <div className="text-gray-600 mt-1">Total brands</div>
            </Card>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{brands.current_page}</span>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{brands.last_page}</span>
