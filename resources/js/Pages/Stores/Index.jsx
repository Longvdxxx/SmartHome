...
export default function Index({ auth, stores, filters }) {
  const [globalFilter, setGlobalFilter] = useState(filters?.search || '');
  const toast = useRef(null);

  const handleSearch = (e) => {
    e.preventDefault();
    router.get(route('stores.index'), { search: globalFilter }, {
      preserveState: true,
      replace: true
    });
  };
  ...
  const confirmDelete = (store) => {
    confirmDialog({
      message: `Are you sure you want to delete store "${store.name}"?`,
  ...
  const actionTemplate = (rowData) => (
    <div className="flex gap-2">
      <Button
        ...
        onClick={() => router.visit(route("stores.inventory", rowData.id))}
      />
      <Button
        icon="pi pi-pencil"
        ...
        tooltip="Edit"
        tooltipOptions={{ position: "top" }}
        onClick={() => router.visit(route("stores.edit", rowData.id))}
      />
      <Button
        icon="pi pi-trash"
  ...
  const nameTemplate = (rowData) => (
    <Link
      href={route("stores.inventory", rowData.id)}
        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <Card className="shadow-3">
            <DataTable
              ...
              lazy
              first={(stores.current_page - 1) * stores.per_page}
            ...
            >
              ...
              <Column field="name" header="Name" body={nameTemplate} sortable style={{ minWidth: '200px' }} />
              <Column field="address" header="Address" sortable style={{ minWidth: '250px' }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <Card className="text-center shadow-2">
              <div className="flex align-items-center justify-content-center mb-3">
                <div className="bg-blue-100 p-3 border-round-xl">
                  <i className="pi pi-store text-blue-500 text-2xl"></i>
                </div>
              </div>
              <span className="text-2xl font-bold text-gray-800">{stores.total}</span>
              <div className="text-gray-600 mt-1">Total stores</div>
            </Card>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{stores.current_page}</span>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{stores.last_page}</span>
