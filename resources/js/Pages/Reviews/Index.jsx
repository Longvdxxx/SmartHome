...
export default function Index({ auth, reviews, filters }) {
  const [globalFilter, setGlobalFilter] = useState(filters?.search || '');
  const toast = useRef(null);

  const handleSearch = (e) => {
    e.preventDefault();
    router.get(route('reviews.index'), { search: globalFilter }, {
      preserveState: true,
      replace: true
    });
  };
  ...
  const confirmDelete = (review) => {
    confirmDialog({
      message: `Are you sure you want to delete this review from "${review.user?.name}"?`,
  ...
  const userTemplate = (rowData) => rowData.user?.name || '-';
  const productTemplate = (rowData) => rowData.product?.name || '-';
        ...
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <Card className="shadow-3">
            ...
            >
              ...
              <Column field="user.name" header="User" body={userTemplate} sortable style={{ minWidth: '150px' }} />
              <Column field="product.name" header="Product" body={productTemplate} sortable style={{ minWidth: '200px' }} />
              <Column field="rating" header="Rating" body={ratingTemplate} sortable style={{ minWidth: '150px' }} />
              <Column field="comment" header="Comment" sortable style={{ minWidth: '250px' }} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <Card className="text-center shadow-2">
              <div className="flex align-items-center justify-content-center mb-3">
                <div className="bg-blue-100 p-3 border-round-xl">
                  <i className="pi pi-comments text-blue-500 text-2xl"></i>
                </div>
              </div>
              <span className="text-2xl font-bold text-gray-800">{reviews.total}</span>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{reviews.current_page}</span>
            ...
            <Card className="text-center shadow-2">
              ...
              <span className="text-2xl font-bold text-gray-800">{reviews.last_page}</span>
